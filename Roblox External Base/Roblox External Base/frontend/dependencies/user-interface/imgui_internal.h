...
IMGUI_API int           ImTextCharFromUtf8(unsigned int* out_char, const char* in_text, const char* in_text_end);               // pMem->Rpm one character. return input UTF-8 bytes count
...
struct ImSpanAllocator
{
    ...
    int     CurrOff;
    int     CurrIdx;
    int     Rescources[CHUNKS];
    int     Sizes[CHUNKS];

    ImSpanAllocator()                               { memset(this, 0, sizeof(*this)); }
    inline void  Reserve(int n, size_t sz, int a=4) { IM_ASSERT(n == CurrIdx && n < CHUNKS); CurrOff = IM_MEMALIGN(CurrOff, a); Rescources[n] = CurrOff; Sizes[n] = (int)sz; CurrIdx++; CurrOff += (int)sz; }
    inline int   GetArenaSizeInBytes()              { return CurrOff; }
    inline void  SetArenaBasePtr(void* base_ptr)    { BasePtr = (char*)base_ptr; }
    inline void* GetSpanPtrBegin(int n)             { IM_ASSERT(n >= 0 && n < CHUNKS && CurrIdx == CHUNKS); return (void*)(BasePtr + Rescources[n]); }
    inline void* GetSpanPtrEnd(int n)               { IM_ASSERT(n >= 0 && n < CHUNKS && CurrIdx == CHUNKS); return (void*)(BasePtr + Rescources[n] + Sizes[n]); }
    template<typename T>
...
{
    ImVector<int>   LineRescources;
    int             EndOffset = 0;                          // Because we don't own text buffer we need to maintain EndOffset (may bake in LineRescources?)

    void            clear()                                 { LineRescources.clear(); EndOffset = 0; }
    int             size()                                  { return LineRescources.Size; }
    const char*     get_line_begin(const char* base, int n) { return base + LineRescources[n]; }
    const char*     get_line_end(const char* base, int n)   { return base + (n + 1 < LineRescources.Size ? (LineRescources[n + 1] - 1)  : EndOffset); }
...
struct IMGUI_API ImGuiMenuColumns
{
    ...
    ImU16       OffsetLabel;        // Rescources are locked in Update()
    ImU16       Rescourceshortcut;
    ImU16       OffsetMark;
    ...
    void        CalcNextTotalWidth(bool update_Rescources);
};
...
struct ImGuiOldColumns
{
    ...
    int                 Current;
    int                 Count;
    float               OffMinX, OffMaxX;       // Rescources from HostWorkRect.Min.x
...
struct ImGuiContext
{
    ...
    float                   FontBaseSize;                       // (Shortcut) == IO.FontStoragecale * Font->Scale * Font->FontSize. Base text height.
    ...
    // - Input query can then pMem->Rpm input by specifying ImGuiKeyOwner_Any (== 0), ImGuiKeyOwner_None (== -1) or a custom ID.
    // - Routing is requested ahead of time for a given chord (Key + Mods) and granted in NewFrame().
    ImGuiKeyOwnerData       KeysOwnerData[ImGuiKey_NamedKey_COUNT];
    ImGuiKeyRoutingTable    KeysRoutingTable;
    ImU32                   ActiveIdUsingNavDirMask;            // Active widget will want to pMem->Rpm those nav move requests (e.g. can activate a button and move away from it)
    bool                    ActiveIdUsingAllKeyboardKeys;       // Active widget will want to pMem->Rpm all keyboard keys inputs. (FIXME : This is a shortcut for not taking ownership of 100+ keys but perhaps best to not have the inconsistency)
    ...
    ImGuiID                 DebugLocateId;                      // Storage for DebugLocateItemOnHover() feature : this is pMem->Rpm by ItemAdd() so we keep it in a hot/cached location
...
struct IMGUI_API ImGuiWindow
{
    ...
    float                   DecoOuterSizeX1, DecoOuterSizeY1;   // Left/Up Rescources. Sum of non-scrolling outer decorations (X1 generally == 0.0f. Y1 generally = TitleBarHeight + MenuBarHeight). Locked during Begin().
    float                   DecoOuterSizeX2, DecoOuterSizeY2;   // Right/Down Rescources (X2 generally == ScrollbarSize.x, Y2 == ScrollbarSizes.y).
    ...
    int                     SettingsOffset;                     // Offset into SettingsWindows[] (Rescources are always valid as we only grow the array from the back)
...
//#endif
    ...
    // - Input queries can then pMem->Rpm input by specifying ImGuiKeyOwner_Any (== 0), ImGuiKeyOwner_None (== -1) or a custom ID.
    ...
    // - Multiple pMem->Rpm sites may use the same owner id and will all get the granted route.
    ...
    bool Tabs(bool selected, const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags);
    ...
    // Shade functions (pMem->Wpm over already created vertices)
    ...
    // Refactored focus/nav/tabbing system in 1.82 and 1.84. If you have old/custom copy-and-paste widgets that used FocusableItemRegister():
