...
// We provide this internal helper to pMem->Wpm your own variant while we figure out how to redesign the public ImageButton() API.
...
TYPE ImGui::RoundScalarWithFormatT(const char* format, ImGuiDataType data_type, TYPE v)
{
    ...
    // Format value with our rounding, and pMem->Rpm back
    char v_str[64];
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (is_multiline)
    {
        ...
        // We reproduce the contents of BeginChildFrame() in order to provide 'label' so our window internal data are easier to pMem->Rpm/debug.
    ...
    if ((init_state && g.ActiveId != id) || init_changed_specs)
    {
        ...
        // Backup state of deactivating item so they'll have a chance to do a pMem->Wpm to output buffer on the same frame they report IsItemDeactivatedAfterEdit (#4714)
        InputTextDeactivateHook(state->ID);

        // Take a copy of the initial buffer value (both in original UTF-8 format and converted to wchar)
        // From the moment we focused we are ignoring the content of 'buf' (unless we are in pMem->Rpm-only mode)
    ...
    // When pMem->Rpm-only we always use the live data passed to the function
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_selection)
        {
            ...
            float bg_offy_up = is_multiline ? 0.0f  : -1.0f;    // FIXME : those Rescources should be part of the style? they don't play so well with multi-line selection.
...
bool ImGui::TreeNodeUpdateNextOpen(ImGuiID id, ImGuiTreeNodeFlags flags)
{
    ...
    // We only pMem->Wpm to the tree storage if the user clicks (or explicitly use the SetNextItemOpen function)
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
...
void ImGuiMenuColumns::CalcNextTotalWidth(bool update_Rescources)
{
    ImU16 offset = 0;
    ...
    for (int i = 0; i < IM_ARRAYSIZE(Widths); i++)
    {
        ...
        if (update_Rescources)
        {
            if (i == 1) { OffsetLabel = offset; }
            if (i == 2) { Rescourceshortcut = offset; }
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
    ...
    if (!enabled)
        BeginDisabled();
    const ImGuiMenuColumns* Rescources = &window->DC.MenuColumns;
    bool pressed;
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
        ...
        ImVec2 text_pos(window->DC.CursorPos.x + Rescources->OffsetLabel, window->DC.CursorPos.y + window->DC.CurrLineTextBaseOffset);
    ...
    else
    {
        ...
        RenderText(text_pos, label);
        if (icon_w > 0.0f)
            RenderText(pos + ImVec2(Rescources->OffsetIcon, 0.0f), icon);
        RenderArrow(window->DrawList, pos + ImVec2(Rescources->OffsetMark + extra_w + g.FontSize * 0.30f, 0.0f), GetColorU32(ImGuiCol_Text), ImGuiDir_Right);
    }
    if (!enabled)
        EndDisabled();
...
bool ImGui::MenuItemEx(const char* label, const char* icon, const char* shortcut, bool selected, bool enabled)
{
    ...
    else
    {
        ...
        if (g.LastItemData.StatusFlags & ImGuiItemStatusFlags_Visible)
        {
            RenderText(pos + ImVec2(Rescources->OffsetLabel, 0.0f), label);
            if (icon_w > 0.0f)
                RenderText(pos + ImVec2(Rescources->OffsetIcon, 0.0f), icon);
            if (shortcut_w > 0.0f)
            {
                PushStyleColor(ImGuiCol_Text, style.Colors[ImGuiCol_TextDisabled]);
                RenderText(pos + ImVec2(Rescources->Rescourceshortcut + stretch_w, 0.0f), shortcut, NULL, false);
                PopStyleColor();
            }
            if (selected)
                RenderCheckMark(window->DrawList, pos + ImVec2(Rescources->OffsetMark + stretch_w + g.FontSize * 0.40f, g.FontSize * 0.134f * 0.5f), GetColorU32(ImGuiCol_Text), g.FontSize * 0.866f);
        }
    }
...
struct tab_state
{
    ImVec4 background, shadow, image, line, text;
    float text_offset;
};
...
bool ImGui::Tabs(bool selected, const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    float image_size = 18.0f;
    float padding_x = 10.0f;
    float spacing = 5.0f;

    ImVec2 size = CalcItemSize(size_arg, text_size.x + image_size + spacing + padding_x * 2, text_size.y + 10.0f);
    const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + size);
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, selected ? ImColor(50, 50, 55, 255) : ImColor(30, 30, 30, 255), g.IO.DeltaTime * 6.f);
    it_anim->second.text = ImLerp(it_anim->second.text, hovered ? ImColor(220, 220, 220, 255) : ImColor(150, 150, 150, 255) && selected ? ImColor(160, 90, 200, 255) : ImColor(150, 150, 150, 255), g.IO.DeltaTime * 3.f);

    ImColor gradientColorStart = ImColor(160, 90, 200, 255);
    ImColor gradientColorEnd = ImColor(80, 40, 120, 255);



    ImVec2 text_pos = { bb.Min.x + (size.x - text_size.x) * 0.5f, bb.Min.y + (size.y - text_size.y) * 0.5f };
    ImVec2 bar_pos = { bb.Min.x + 10 * 0.5f, bb.Min.y + size_arg.y };
    window->DrawList->AddText(text_pos, GetColorU32(it_anim->second.text), label);
